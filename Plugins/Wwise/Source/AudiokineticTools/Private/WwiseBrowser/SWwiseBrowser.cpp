...
void SWwiseBrowser::Construct(const FArguments& InArgs)
{
	...
	ChildSlot
		...
		[
			SNew(SSearchBox)
			.HintText(LOCTEXT("WwiseBrowserSearchTooltip", "Search Wwise Item"))
		...
		[
			SNew(SBox)
			.Padding(FMargin(5.0f,0.0f,30.0f,0.0f))
			[
				SNew(SButton)
...
void SWwiseBrowser::RestoreTreeExpansion(const TArray< FWwiseTreeItemPtr >& Items)
{
	...
	for(int i = 0; i < Items.Num(); i++)
	{
		...
		if(!DataSource->IsKeptInBrowser(Items[i]))
		{
			continue;
		}
		// If filtering, everything should be expanded?

		if (LastExpandedPaths.Contains(Items[i]->FolderPath) || IsFiltering())
		{
			ExpandItem(Items[i], true);
		}
